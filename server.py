...
class ScrapeRequest(BaseModel):
    url: str
    novel: str        # novel root folder name  (under BASE_DIR)
    volume: str       # volume sub-folder name
    filename: str     # output .md filename (e.g. "Chapter 1.md")
...
def safe_path(rel: str) -> Path:
    ...
    # [Bug 1 fix] Prevent API access to core tool files and directories
    try:
...
def log_event(level: str, message: str):
    ...
    # [Bug 5 fix] Append efficiently, only truncate if file exceeds max_logs by a buffer of 100
    try:
...
# ══════════════════════════════════════════════════════════════════════════════
# ROUTES – Library / Project Structure
# ══════════════════════════════════════════════════════════════════════════════
...
def get_files(novel: str, volume: str):
    return {
        "md_files": list_md_files(novel, volume),
        "images": list_images(novel, volume),
    }
...
def create_volume(req: CreateVolumeRequest):
    ...
    if not target_main.exists():
        ...
        if existing_vols:
            ...
            content = re.sub(r'Table of Content\[[\s\S]*', 'Table of Content[\n]\n', content)
            content = re.sub(r'^Volume\s*:\s*\[.*?\]', 'Volume: []', content, flags=re.MULTILINE)
...
def scrape_url(req: ScrapeRequest):
    ...
    try:
        ...
        # 1. Decode KDT Novels canvas protection
        import base64
        ...
        # 2. Generic Lazy-Load Image fallback
...
def build_epub(req: BuildRequest):
    ...
    try:
        ...
        # Try to find the output EPUB path
        epub_path = None
...
# ══════════════════════════════════════════════════════════════════════════════
# ROUTES – main.md metadata (volume-level)
# ══════════════════════════════════════════════════════════════════════════════
...
def save_mainmd(req: SaveMdRequest):
    # req.path should be like "Novel/Volume/main.md"
    p = safe_path(req.path)
    p.write_text(req.content, encoding="utf-8")
    return {"ok": True}
...
class BatchScrapeRequest(BaseModel):
    urls: list       # list of {"url": str, "filename": str}
    novel: str
    volume: str
...
def batch_scrape(req: BatchScrapeRequest):
    ...
    for item in req.urls:
        single = ScrapeRequest(
            ...
            novel=req.novel,
            volume=req.volume,
            filename=item.get("filename", "chapter.md")
        )
        try:
            r = scrape_url(single)
