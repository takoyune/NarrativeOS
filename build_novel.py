...
def load_settings():
    ...
    return {
        # [image]
        'image_format': config.get('image', 'format', fallback='webp').lower(),
        'image_quality': config.getint('image', 'quality', fallback=90),
        'max_size_mb': config.getfloat('image', 'max_size_mb', fallback=1.5),
        'max_dimension': config.getint('image', 'max_dimension', fallback=2000),
        'webp_method': config.getint('image', 'webp_method', fallback=4),
        # [output]
        'language': config.get('output', 'language', fallback='id'),
        'output_location': config.get('output', 'output_location', fallback='parent'),
        'output_name_format': config.get('output', 'output_name_format', fallback='{folder_name}'),
        # [styling]
        'drop_cap': config.getboolean('styling', 'drop_cap', fallback=False),
        'scene_break_symbol': config.get('styling', 'scene_break_symbol', fallback='❖ ❖ ❖'),
        'font_size': config.getfloat('styling', 'font_size', fallback=1.0),
        'line_height': config.getfloat('styling', 'line_height', fallback=1.8),
        'text_indent': config.getfloat('styling', 'text_indent', fallback=1.5),
        # [build]
        'auto_convert_images': config.getboolean('build', 'auto_convert_images', fallback=True),
        'auto_compress': config.getboolean('build', 'auto_compress', fallback=True),
        'download_online_images': config.getboolean('build', 'download_online_images', fallback=True),
        'download_retries': config.getint('build', 'download_retries', fallback=3),
        'download_timeout': config.getint('build', 'download_timeout', fallback=30),
    }
...
def compress_image(filepath, max_mb=None):
    ...
    try:
        ...
        # Downscale if very large, keeping aspect ratio
        ...
        # If file is already target format and small enough, skip
        ...
        if pil_format in ('WEBP',) and img.mode in ('RGBA', 'LA', 'P'):
            ...
            img_save = img.convert('RGB')  # JPEG doesn't support alpha
        else:
            ...
            for file in list(os.listdir(folder_path)):  # list() avoids issues when files are renamed
                ...
                if is_image and not is_target_format and SETTINGS['auto_convert_images']:
                    ...
                    if new_path != filepath:
                        # Image was converted — sync all .md references
                        update_md_image_references(
                            base_dir,
...
# Add the directory to sys.path so we can import epub_builder
sys.path.append(os.path.dirname(os.path.abspath(__file__)))
import epub_builder

def process_markdown_tricks(md_text):
    scene_break_symbol = SETTINGS['scene_break_symbol']
    # Scene breaks: *** or --- on a new line
    md_text = re.sub(r'^\s*(\*\*\*|---)\s*$', f'\n<div class="scene-break">{scene_break_symbol}</div>\n', md_text, flags=re.MULTILINE)
    
    # Thought block on its own line: (thought) text or (thought) [text]
    md_text = re.sub(r'^\s*\(thought\)\s*\[?(.*?)\]?\s*$', r'<div class="thought-box">\1</div>', md_text, flags=re.IGNORECASE | re.MULTILINE)
    # Thought inline: (thought) [text]
    md_text = re.sub(r'\(thought\)\s*\[(.*?)\]', r'<span class="thought-inline">\1</span>', md_text, flags=re.IGNORECASE)
    
    # Stats box: [stats] ... [/stats]
    # Replaces with a blockquote so CSS can style it as a UI box
    md_text = re.sub(r'\[stats\](.*?)\[/stats\]', r'\n<blockquote>\n\1\n</blockquote>\n', md_text, flags=re.DOTALL | re.IGNORECASE)
...
def add_dropcap(html):
    # Find the first <p> that starts with a letter and add a styled drop cap span
    match = re.search(r'<p>([A-Za-z])(.*?</p>)', html, flags=re.DOTALL)
    if match:
...
def remove_first_p_indent(html):
    match = re.search(r'<p\b([^>]*)>', html)
    if match:
        attrs = match.group(1)
        if 'class=' in attrs:
            new_attrs = re.sub(r'class=["\'](.*?)["\']', r'class="\1 no-indent"', attrs)
        else:
...
def download_online_images(base_dir, md_text):
    ...
    def replacer(match):
        ...
        # Parse URL to remove query params like ?w=730
        parsed = urllib.parse.urlparse(url)
        clean_url = urllib.parse.urlunparse((parsed.scheme, parsed.netloc, parsed.path, '', '', ''))
        
        filename = os.path.basename(parsed.path)
        if not filename:
            filename = "downloaded_img.jpg"  # fallback
        # [Upgrade 3] Sanitize: replace spaces and special chars with underscores
        ...
        if not os.path.exists(local_path):
            ...
            # [Upgrade 1] Use retry-enabled download_url instead of raw urlopen
...
def build_novel(args=None):
    ...
    parser.add_argument("folder", nargs="?", default=r"d:\code iwan\EPUB\Teori Guru Jenius Berengsek tentang Cara Membesarkan Putri Bangsawan yang Jatuh ke Jalan Kegelapan", help="Path to the novel folder")
    ...
    # Extract metadata
    title_indo = re.search(r"Title Indonesia\s*:\s*\[(.*?)\]", content, flags=re.IGNORECASE)
    title_en = re.search(r"Title Inggris\s*:\s*\[(.*?)\]", content, flags=re.IGNORECASE)
    title_romaji = re.search(r"Title Romanji\s*:\s*\[(.*?)\]", content, flags=re.IGNORECASE)
    ...
    # Extract all metadata pairs for the About Page before the Table of Content section
    metadata_section = content.split("Table of Content")[0]
    metadata_pairs = re.findall(r"^([^:\n]+?)\s*:\s*\[(.*?)\]", metadata_section, flags=re.MULTILINE)
    
    # Check if About is in TOC
    has_about = re.search(r"\[About\]", content, flags=re.IGNORECASE) is not None
    about_body_html = ""
    if has_about:
        about_body_elements = []
        ...
        # Display main title as a primary highlight
    ...
    try:
        ...
        def add_image_to_manifest(filename, src_path, is_cover=False):
            ...
            # Convert non-WebP images to WebP on the fly
            ...
            if ext in ('png', 'jpg', 'jpeg') and Image is not None:
                ...
                if not os.path.exists(dest_path):
                    try:
                        ...
                        webp_filename = filename  # fallback to original
...
{body}
        ...
        # 1. Handle Global Cover Image (metadata cover)
        if cover_rel_path:
            if cover_rel_path.lower().startswith(('http://', 'https://')):
                src_images_dir = os.path.join(base_dir, "images")  # source dir, NOT images_dir (temp)
                ...
                # [Bug 3 fix] Compress cover if oversized
                ...
                # Robust fallback check across different potential folder names
                if not os.path.exists(cover_src):
        ...
        # 2. Process TOC items in exact sequential order
        chapter_idx = 1
        # [Bug 5 fix] Directory prefix is now optional — matches "images/file.jpg" AND bare "file.jpg"
        inline_img_pattern = re.compile(r"\(image\)\s*\[(?:.*?[\/\\])?([^\/\]\\ ]+\.(?:webp|jpg|jpeg|png))\]", re.IGNORECASE)

        # Extract TOC block and its items in order (stops at the closing bracket on its own line to avoid comments)
        ...
        for item in toc_items:
            ...
            if cover_match:
                ...
                if actual_c_filename:
                    ...
                    manifest_items.append({
                        'id': 'cover_page',
                        'href': f'xhtml/{out_filename}',
                        'media-type': 'application/xhtml+xml'
                    })
                    spine_items.append({'id': 'cover_page', 'title': 'Cover'})
                continue

            # Case 2: Illustration Folder (legacy inline illustrations)
            ...
            # Case 5: Chapter File
            # [Bug 6 fix] \s* tolerates optional space between filename and (file)
            ...
            if chapter_match:
                ...
                if os.path.exists(file_path):
                    ...
                    # Auto-download online images and convert to local image tags
                    ...
                    # 1. Get real title from text
                    ch_title = filename.replace('.md', '').replace('.MD', '')
                    first_heading = re.search(r'^#\s+(.*)', md_text, flags=re.MULTILINE)
                    if first_heading:
                    ...
                    # 2. Split chapter by inline image pattern
                    ...
                    # Filter out empty/whitespace-only text segments
                    filtered_segments = []
                    ...
                    # If no valid segments, continue
                    if not filtered_segments:
                        continue

                    # Check if chapter is image-only (has no non-empty text segments)
                    ...
                    for seg_type, val in filtered_segments:
                        if seg_type == 'text':
                            ...
                            if not first_text_segment_done:
                                ...
                                # Continuation text segment — remove indent of first paragraph
                            ...
                            # Only the very first part of the chapter should be in the TOC
                            ...
                            if actual_img:
                                ...
                                # The first page of the file (even if it's an image) is included in the TOC
                                spine_item = {
                                    'id': ch_id,
                                    'title': ch_title if is_first_part else 'Ilustrasi'
                                }
                                if not is_first_part:
                                    spine_item['exclude_from_toc'] = True
                                spine_items.append(spine_item)
        ...
        try:
            vol_match = re.search(r'Volume\s*(\d+)', novel_folder_name, flags=re.IGNORECASE)
        ...
        # [Upgrade 2] Build summary
        epub_size_mb = os.path.getsize(output_epub) / 1024 / 1024
        ch_count = sum(1 for s in spine_items
                       if isinstance(s, dict) and s.get('id', '').endswith('_01') and s.get('id', '').startswith('ch'))
